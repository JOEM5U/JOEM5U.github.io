...
let loop = document.getElementsByClassName("loop")[0];
...
/*This is for the audio slider for the video/media bar, which we went through class.
For this I believe that it is an important function for relaxation/ studying as people have different audio levels which they would like to listen to. 
This helps as there may be those that would want to listen to more quiter audio while there are those that would like it louder to block out other distracting sounds.
This feature helpsin achieving that balance for various users, instead of giving them one audio level to listen to.*/

/*For loop button */ 
loop.addEventListener("click", loopAudio);

function loopAudio(){
  media.toggleAttribute('loop');
}
/*Here I decided to add function tp the click which would make thew audio loop.
I will explain the reason within my html sheet */